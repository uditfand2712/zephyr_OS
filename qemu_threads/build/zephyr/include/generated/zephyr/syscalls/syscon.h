...
extern int z_impl_syscon_read_reg(const struct device * dev, uint32_t reg, uint32_t * val);

__pinned_func
static inline int syscon_read_reg(const struct device * dev, uint32_t reg, uint32_t * val)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		union { uintptr_t x; const struct device * val; } parm0 = { .val = dev };
		union { uintptr_t x; uint32_t val; } parm1 = { .val = reg };
...
extern int z_impl_syscon_write_reg(const struct device * dev, uint32_t reg, uint32_t val);

__pinned_func
static inline int syscon_write_reg(const struct device * dev, uint32_t reg, uint32_t val)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		union { uintptr_t x; const struct device * val; } parm0 = { .val = dev };
...
extern int z_impl_syscon_update_bits(const struct device * dev, uint32_t reg, uint32_t mask, uint32_t val);

__pinned_func
static inline int syscon_update_bits(const struct device * dev, uint32_t reg, uint32_t mask, uint32_t val)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		union { uintptr_t x; const struct device * val; } parm0 = { .val = dev };
